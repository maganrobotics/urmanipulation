from panda3d.core import PerspectiveLens, OrthographicLens, AmbientLight, PointLight, Vec4, Vec3, Point3, \
    WindowProperties, Filename, NodePath, Shader, GraphicsPipe, FrameBufferProperties, GraphicsOutput,GraphicsWindow
...
class World(ShowBase, object):
    ...
    def __init__(self,
        ...
        super().__init__()
        # set up window
        winprops = WindowProperties(base.win.getProperties()) # this  can not add in Panda3DWorld
        #winprops = WindowProperties()
        #winprops.setTitle("WRS Robot Planning and Control System")
        winprops.setTitle("SZTU Robotics Laboratory")
        ...
        #ShowBase.taskMgr.add(self._interaction_update, "interaction", appendTask=True)
        # set up rotational cam
        if auto_cam_rotate:
            taskMgr.doMethodLater(.1, self._rotatecam_update, "rotate cam")
            #ShowBase.taskMgr.doMethodLater(.1, self._rotatecam_update, "rotate cam")
        # set window size
        ...
        #ShowBase.taskMgr.add(self._physics_update, "physics", appendTask=True)
        ...
        #ShowBase.taskMgr.add(self._internal_update, "internal_update", appendTask=True)
        ...
        #ShowBase.taskMgr.add(self._external_update, "external_update", appendTask=True)
