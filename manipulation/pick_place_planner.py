...
class PickPlacePlanner(adp.ADPlanner):
    ...
    def gen_static_motion(self, the_other_conf_list, conf, jawwidth):
        """

        Parameters
        ----------
        the_other_conf_list: the length of the other arm conf list
        conf: the conf that you want to keep
        jawwidth: the jawwidth that you want to keep
        ...
        """
        static_conf_list = []
        static_jawwidth_list = []
        for _ in the_other_conf_list:
            static_conf_list.append(conf)
            static_jawwidth_list.append(jawwidth)
        return static_conf_list, static_jawwidth_list

    def gen_static_motion_with_range(self, conf_range, conf, jawwidth):
        """
        ...
        Returns :static_conf_list, static_jawwidth
        -------
    ...
    def find_common_graspids(self,
        ...
        for goalid, goal_homomat in enumerate(goal_homomat_list):
            ...
            for graspid, grasp_info in graspid_and_graspinfo_list:
                # this is used to know the connection about grasp_info
                print(graspid)
                print(grasp_info)
            ...
            print("intermediate_available_graspids", intermediate_available_graspids)
            print('-----start-----')
            ...
            print('------end------')
        final_available_graspids = previously_available_graspids  # the final useful grasp ids
        self.robot_s.fk(hand_name, jnt_values_bk)
        print("final_available_graspids", final_available_graspids)
        return final_available_graspids, intermediate_available_graspids
    ...
    def gen_holding_moveto(self,
        ...
        # hold object
        jaw_width, jaw_center_pos, jaw_center_rotmat, hnd_pos, hnd_rotmat = grasp_info
        print('in gen_holding_moveto', grasp_info)
        first_obj_pos = obj_pose_list[0][:3, 3]
        first_obj_rotmat = obj_pose_list[0][:3, :3]
        first_jaw_center_pos = first_obj_rotmat.dot(jaw_center_pos) + first_obj_pos  # 公式的意义
        ...
        print("the first conf is", first_conf )
    ...
    def gen_pick_and_place_motion(self,
        ...
        for grasp_id in common_grasp_id_list:
            print("the grasp id", grasp_id)
            grasp_info = grasp_info_list[grasp_id]
            print("the common grasp id", common_grasp_id_list)
            print("in gen_pick_and_place_motion", grasp_info)
            jaw_width, jaw_center_pos, jaw_center_rotmat, hnd_pos, hnd_rotmat = grasp_info
            # approach
