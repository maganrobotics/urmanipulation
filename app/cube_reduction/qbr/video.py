    ...
    def run(self):
        ...
        while True:
            ...
            if not self.calibrate_mode:
                ...
                if key == 32:
                    self.update_snapshot_state(frame)  # 保存快照

                # Switch to another language.
                if key == ord(SWITCH_LANGUAGE_KEY):  # 切换语言
            ...
            if key == ord(CALIBRATE_MODE_KEY):  # 进入校准模式
            ...
            contours = self.find_contours(dilatedFrame)  # 查找正方形块
            if len(contours) == 9:
                self.draw_contours(frame, contours)  # 画出正方形
                if not self.calibrate_mode:  # 不是校准模式
                    self.update_preview_state(frame, contours)  # 识别模式 实时预览识别到的色块
                elif key == 32 and self.done_calibrating == False:  # 按下空格 且是 校准模式
                    current_color = self.colors_to_calibrate[self.current_color_to_calibrate_index]
                    (x, y, w, h) = contours[4]  # 获取中间色块的坐标
                    roi = frame[y+7:y+h-7, x+14:x+w-14]
                    avg_bgr = color_detector.get_dominant_color(roi)  # 获取中心色块 rgb
                    self.calibrated_colors[current_color] = avg_bgr
                    self.current_color_to_calibrate_index += 1
                    self.done_calibrating = self.current_color_to_calibrate_index == len(self.colors_to_calibrate)  # 判断是否完成所有面的校准
                    if self.done_calibrating:
            ...
            if self.calibrate_mode:  # 校准模式
                self.draw_current_color_to_calibrate(frame)  # 显示需要校准的色块
                self.draw_calibrated_colors(frame)  # 显示已校准的颜色
            else:
